...
document.getElementById('loginForm').addEventListener('submit', function(event) {
    ...
    fetch('http://localhost:8080/login', {
        ...
        body: `conta=${encodeURIComponent(username)}&senha=${encodeURIComponent(password)}`,
        credentials: 'include'
    })
        .then(response => {
            if (response.ok) {
                ...
                document.getElementById('itensListSection').style.display = 'block'; // Mostra a seção de itens
                document.querySelector('.navbar').style.display = 'flex'; // Mostra a barra de navegação
            } else {
